...
void termMove(int x, int y) // déplace le curseur du terminal
{
    #ifdef _WIN32
...
void termInit()      // configure la saisie : ne pas afficher les caractères tapés
{
    #ifdef _WIN32
...
char WinTXT::getCh()
{
    // lire un caractère si une touche a été pressée
    char touche=0;
#ifdef _WIN32
    if (kbhit())
    {
